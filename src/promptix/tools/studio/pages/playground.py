...
def render_model_config():
    ...
    with col1:
        model = st.selectbox(
            "Model", ["gpt-4", "gpt-3.5-turbo", "gpt-4-turbo"], index=0
        )
    ...
    with col3:
        max_tokens = st.number_input(
            "Max Tokens", min_value=1, max_value=4096, value=1024
        )
...
def render_system_prompt():
    ...
    system_prompt = st.text_area(
        "System Message", value="You are a helpful AI assistant.", height=100
    )
...
def render_test_interface():
    ...
    user_input = st.text_area(
        "User Message", placeholder="Enter a test message...", height=100
    )
...
def render_playground():
    ...
    if st.button("💾 Save All Changes", use_container_width=True):
        version_data = {"config": config, "system_prompt": system_prompt}
