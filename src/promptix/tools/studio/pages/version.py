...
def render_version_list(prompt: Dict):
    ...
    sorted_versions = sorted(
        versions.items(), key=lambda x: x[1].get("created_at", ""), reverse=True
    )
    ...
    for version_id, version_data in sorted_versions:
        ...
        is_first = version_id == sorted_versions[0][0]
        with st.expander(f"Version {version_id}", expanded=is_first):
            ...
            with col2:
                if st.button(
                    "📝 Edit", key=f"edit_{version_id}", use_container_width=True
                ):
            ...
            with col3:
                if st.button(
                    button_label, key=f"go_live_{version_id}", use_container_width=True
                ):
                    if not is_live:  # Only update if not already live
                        ...
                        prompt_manager.save_prompt(
                            st.session_state["prompt_id"], prompt
                        )
                    st.rerun()
...
def render_new_version():
    ...
    with col1:
        new_version = st.text_input(
            "Version Name", placeholder="e.g., v1, production, test, etc."
        )
