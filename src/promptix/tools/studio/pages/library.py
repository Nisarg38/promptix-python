...
def render_prompt_list():
    ...
    search_query = st.text_input(
        "🔍", placeholder="Search prompts...", label_visibility="collapsed"
    )
    ...
    for prompt_id, prompt in prompts.items():
        if (
            search_query.lower() in prompt["name"].lower()
            or search_query.lower() in prompt.get("description", "").lower()
        ):
